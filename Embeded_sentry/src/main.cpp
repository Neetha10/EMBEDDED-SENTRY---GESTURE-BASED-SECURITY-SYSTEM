...
#define CORRELATION_THRESHOLD 3.0f
 ...
 * ****************************************************************************/
void draw_circle_button(int center_x, int center_y, int radius, const char *label);
bool is_touch_inside_circle(int touch_x, int touch_y, int center_x, int center_y, int radius);
void show_recording_progress(float elapsed_time);
void draw_success_emoji(int center_x, int center_y);
void draw_failed_emoji(int center_x, int center_y);
...
const int button1_center_x = 120;
const int button1_center_y = 105;
const int button_radius = 50;
const char *button1_label = "RECORD";
const int button2_center_x = 120;
const int button2_center_y = 205;
const char *button2_label = "UNLOCK";
const int message_x = 5;
const int message_y = 30;
const char *message = "SENTRY";
const int text_x = 5;
...
int main()
{
    ...
    // Draw button 1
    draw_circle_button(button1_center_x, button1_center_y, button_radius, button1_label);

    // Draw button 2
    draw_circle_button(button2_center_x, button2_center_y, button_radius, button2_label);
...
void show_recording_progress(float elapsed_time) {
    const int BAR_X = 20;               // X position of the bar
    const int BAR_Y = 80;               // Y position start
    const int BAR_WIDTH = 20;           // Width of bar 
    const int BAR_HEIGHT = 150;         // Height of bar
    const int BORDER_THICKNESS = 2;     // Thickness of border

    // Display time
    char time_str[10];
    sprintf(time_str, "%.1fs", elapsed_time * 5.0f);
    lcd.SetTextColor(LCD_COLOR_WHITE);
    lcd.FillRect(BAR_X - 10, BAR_Y - 25, 50, 20);  // Clear previous time
    lcd.SetTextColor(LCD_COLOR_BLACK);
    lcd.DisplayStringAt(BAR_X - 10, BAR_Y - 25, (uint8_t *)time_str, LEFT_MODE);

    // Calculate progress (0 to 1)
    float progress = elapsed_time;
    if (progress > 1.0f) progress = 1.0f;
    if (progress < 0.0f) progress = 0.0f;

    int fill_height = (int)(progress * BAR_HEIGHT);

    // Clear previous bar
    lcd.SetTextColor(LCD_COLOR_WHITE);
    lcd.FillRect(BAR_X-BORDER_THICKNESS, BAR_Y-BORDER_THICKNESS, 
                 BAR_WIDTH+2*BORDER_THICKNESS, BAR_HEIGHT+2*BORDER_THICKNESS);

    // Draw progress bar background
    lcd.SetTextColor(LCD_COLOR_LIGHTGRAY);
    lcd.FillRect(BAR_X, BAR_Y, BAR_WIDTH, BAR_HEIGHT);

    // Draw the filled portion (bottom to top)
    if (fill_height > 0) {
        lcd.SetTextColor(LCD_COLOR_RED);
        lcd.FillRect(BAR_X, BAR_Y + BAR_HEIGHT - fill_height, 
                     BAR_WIDTH, fill_height);
    }

    // Draw border
    lcd.SetTextColor(LCD_COLOR_BLACK);
    lcd.DrawRect(BAR_X-BORDER_THICKNESS, BAR_Y-BORDER_THICKNESS, 
                 BAR_WIDTH+2*BORDER_THICKNESS, BAR_HEIGHT+2*BORDER_THICKNESS);
}
/*************************************************************
 * SUCCESS EMOJI
...
void draw_success_emoji(int center_x, int center_y) {
    ...
    lcd.SetTextColor(LCD_COLOR_WHITE);
    lcd.FillRect(center_x - 30, center_y - 30, 60, 60);
    
    // Draw yellow circle face
    lcd.SetTextColor(LCD_COLOR_YELLOW);
    for(int y = -20; y <= 20; y++) {
        for(int x = -20; x <= 20; x++) {
            if(x*x + y*y <= 400) {  // 20*20
                lcd.DrawPixel(center_x + x, center_y + y, LCD_COLOR_YELLOW);
            }
        }
    }
    ...
    // Draw black outline
    lcd.SetTextColor(LCD_COLOR_BLACK);
    lcd.DrawCircle(center_x, center_y, 20);
    
    // Draw eyes
    lcd.FillCircle(center_x - 8, center_y - 5, 3);
    lcd.FillCircle(center_x + 8, center_y - 5, 3);
    
    // Draw upward curved smile
    lcd.SetTextColor(LCD_COLOR_BLACK);
    for(int x = -10; x <= 10; x++) {
        int y = -(x * x) / 20;  // Negative makes curve go upward
        lcd.DrawPixel(center_x + x, center_y + 7 + y, LCD_COLOR_BLACK);
        lcd.DrawPixel(center_x + x, center_y + 6 + y, LCD_COLOR_BLACK);
    }
}
...
void draw_failed_emoji(int center_x, int center_y) {
    ...
    // Draw sad eyes (downward angles)
    lcd.SetTextColor(LCD_COLOR_BLACK);
    for(int i = 0; i < 3; i++) {
        lcd.DrawLine(center_x - 12 + i, center_y - 8, center_x - 4 + i, center_y - 2);  // Left eye
        lcd.DrawLine(center_x + 4 + i, center_y - 2, center_x + 12 + i, center_y - 8);  // Right eye
    }
    
    // Draw downward curved frown
    for(int x = -10; x <= 10; x++) {
        int y = (x * x) / 20;  // Positive makes curve go downward
        lcd.DrawPixel(center_x + x, center_y + 10 + y, LCD_COLOR_BLACK);
        lcd.DrawPixel(center_x + x, center_y + 9 + y, LCD_COLOR_BLACK);
    }
}
/*******************************************************************************
 *
...
void gyroscope_thread()
{
    ...
    while (1)
    {
        ...
        if (flag_check & ERASE_FLAG)
        {
            ...
            lcd.SetTextColor(LCD_COLOR_WHITE);                  // Set the color to the background color
            lcd.FillRect(0, text_y, lcd.GetXSize(), FONT_SIZE); // Clear a specific line
            lcd.SetTextColor(LCD_COLOR_BLACK);                   // Reset the text color
        ...
        if (flag_check & (KEY_FLAG | UNLOCK_FLAG))
        {
         ...
         // gyro data recording loop
timer.start();
chrono::microseconds start_time = timer.elapsed_time();
...
while (timer.elapsed_time() < 5s)
{
    ...
    // Calculate progress (0 to 1)
    chrono::microseconds current_time = timer.elapsed_time();
    float progress = (current_time - start_time).count() / 5000000.0f;
    
    // Show progress bar
    show_recording_progress(progress);
...
void touch_screen_thread()
{
   ...
   while (1) {
        ...
        if (ts_state.TouchDetected) {
            ...
            if (is_touch_inside_circle(touch_x, touch_y, button1_center_x, button1_center_y, button_radius)) {
                sprintf(display_buffer, "Unlocking Initiated...");  // This is for UNLOCK
                lcd.SetTextColor(LCD_COLOR_WHITE);                  
                lcd.FillRect(0, text_y, lcd.GetXSize(), FONT_SIZE); 
                lcd.SetTextColor(LCD_COLOR_BLACK);                   
                lcd.DisplayStringAt(text_x, text_y, (uint8_t *)display_buffer, CENTER_MODE);
                ThisThread::sleep_for(1s);
                flags.set(UNLOCK_FLAG);  // Set UNLOCK flag
            }

            // Check if the touch is inside unlock button
            if (is_touch_inside_circle(touch_x, touch_y, button2_center_x, button2_center_y, button_radius)) {
                sprintf(display_buffer, "Recording Initiated..."); // This is for RECORD
                lcd.SetTextColor(LCD_COLOR_WHITE);                  
                lcd.FillRect(0, text_y, lcd.GetXSize(), FONT_SIZE); 
                lcd.SetTextColor(LCD_COLOR_BLACK);                   
                lcd.DisplayStringAt(text_x, text_y, (uint8_t *)display_buffer, CENTER_MODE);
                ThisThread::sleep_for(1s);
                flags.set(KEY_FLAG);  // Set RECORD flag
            }
        }
...
/*******************************************************************************
 *
 * @brief draw circular button
 * @param center_x: x coordinate of button center
 * @param center_y: y coordinate of button center
    ...
    for(int y = -radius; y <= radius; y++) {
        for(int x = -radius; x <= radius; x++) {
            if(x*x + y*y <= radius*radius) {
                lcd.DrawPixel(center_x + x, center_y + y, LCD_COLOR_ORANGE);
            }
        }
    }
    
    // Calculate text position to center it in the circle
    int text_width = strlen(label) * 38;
    int text_x = center_x - (text_width / 2);
    int text_y = center_y - 8;
    
    // Display the label
    lcd.SetTextColor(LCD_COLOR_BLACK);
    lcd.DisplayStringAt(text_x, text_y, (uint8_t *)label, CENTER_MODE);
}
/*******************************************************************************
 *
 * @brief Check if the touch point is inside the circular button
